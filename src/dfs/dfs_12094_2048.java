...
public class dfs_12094_2048 {
    ...
    private static void dfs(int cnt) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (isSameBoard(temp, map)) continue;
            if (isMeaningLessToForward(cnt)) continue;
    ...
    private static boolean isSameBoard(int[][] a, int[][] b) {
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++) {
    ...
    private static boolean isMeaningLessToForward(int cnt) {
        return (ans * (1 << (10 - cnt))) <= ans;
    }
